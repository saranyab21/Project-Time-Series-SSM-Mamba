...
class CFG:
    ...
    WIN_MS = int(os.environ.get("WIN_MS", "30"))   # use 30 later with cache
    STEP_MS = int(os.environ.get("STEP_MS", "15"))

    FEATURE_SET = os.environ.get("FEATURE_SET", "efficient").lower()  # minimal|comprehensive|efficient
...
# TSFresh
# ======================
def extract_tsfresh(long_df: pd.DataFrame, feature_set: str, n_jobs: int) -> pd.DataFrame:
    fc = ComprehensiveFCParameters() if feature_set == "comprehensive" else MinimalFCParameters()
    X = extract_features(
...
def main():
    ...
    if X_left_path.exists() and X_right_path.exists():
        ...
        print("Extracting TSFresh features (LEFT)...")
        X_left = extract_tsfresh(left_long, CFG.FEATURE_SET, CFG.N_JOBS)
        X_left.to_parquet(X_left_path)
        print(f"Saved: {X_left_path}  shape={X_left.shape}")

        print("Extracting TSFresh features (RIGHT)...")
        X_right = extract_tsfresh(right_long, CFG.FEATURE_SET, CFG.N_JOBS)
        X_right.to_parquet(X_right_path)
        print(f"Saved: {X_right_path}  shape={X_right.shape}")
